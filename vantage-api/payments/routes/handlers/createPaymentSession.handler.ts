...
export const createPaymentSessionHandler = async (
  ...
  try {
    ...
    // Terminate any existing active Cashfree orders
    const existingActiveOrders = await CashfreeOrderModel.find({
      orderId: order._id,
      isActive: true,
    }).lean();

    for (const existingOrder of existingActiveOrders) {
      try {
        console.log(
          `[PaymentSession] Terminating old Cashfree order: ${existingOrder.cfOrderId}`
        );
        await cashfreeClient.terminateOrder(existingOrder.cfOrderId);
        ...
        await CashfreeOrderModel.updateOne(
          { _id: existingOrder._id },
          {
            $set: {
              ...
              deactivationReason: "terminated_for_new_payment_session",
              terminatedAtCF: new Date(),
            },
          }
        );
      } catch (error) {
        console.error(
          `[PaymentSession] Failed to terminate order ${existingOrder.cfOrderId}:`,
          error
        );
        // Still mark as inactive locally even if Cashfree termination fails
        await CashfreeOrderModel.updateOne(
          { _id: existingOrder._id },
          {
            $set: {
              ...
              lastUpdatedAt: new Date(),
              deactivatedAt: new Date(),
              deactivationReason: "marked_inactive_locally",
              terminationError:
